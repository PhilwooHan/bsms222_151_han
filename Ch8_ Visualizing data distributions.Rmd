---
title: "Chapter 8 Visualizing data distrions"
author: "Philwoo, Han"
output:
  html_document:
    df_print: paged
...
Our first data visualization building block is learning to summarize lists of factors or numeric vectors. More often than not, the best way to share or explore this summary is through data visualization. The most basic statistical summary of a list of objects or numbers is its distribution. Once a vector has been summarized as a distribution, there are several data visualiation techniques to effectively relay this information.

In this chapter, we first discuss properties of a variety of distributions and how to visualize distributions using a motivating example of stuedent heights. We then discuss the *ggplot2* geometries for these visualizations in Section 8.16.

## 8.1 Variable types

We will be working with two types of variables: categorical and numeric. Each can be divided into two other groups: categorical can  e ordinal or not, whereas numerical variables can be discrete or continous.
...
Examples of numerical data are population sizes, murder rates, and heights. Some numerical data can be treated as ordered categorical. We can further divide numerical data into continous and discrete. Continous variables are those that can take any value, such as heights, if measured with enough precision. For example, a pair of twins may be 68.12 and 68.11 inches, repectively. Counts, such as population sizes, are discrete because they have to be round numbers.1
...
To the closes 5%, what proportion of the states are in the North Central region
answer: 23%
...
Rather than using data, the normal distribution is defined with a mathematical formula. For any interval $(a,b)$ , the proportion of values in that interval can be computed using this formula: $$\mbox{Pr}(a < x < b) = \int_a^b \frac{1}{\sqrt{2\pi}s} e^{-\frac{1}{2}\left( \frac{x-m}{s} \right)^2} \, dx$$
...
pnorm(-1.96)
```
The inverse function $\Phi^{-1}(x)$ ives us the **theoretical quantiles** for the normal distribution. So, for example, $\Phi^{-1}(0.975) = 1.96$. In R, we can evaluate the inverse of $\phi$ using the qnorm function.
```{r}
qnorm(0.975)
```
...
To see if they match or not, we plot them against each other and draw the identitiy line:
```{r}
qplot(theoretical_quantiles, sample_quantiles) + geom_abline()
```

Notic that this code becomes much cleaner if we use standard units:
...
3. Sutdy the following boxplots showing population sizes by country:
...
7. If we use a log transformation, which continent show n above has the largest interquartile range?
America

8. Load the height data set and creat a vector x with just the male heights:
```{r}
library(dslabs)
data(heights)
...
What porportion of the data is between 69 and 72 inches (taller than 69, but shorter or equal to 72)? Hint: use a logical operator and mean.
...
```{r}
150 / (1000000000* (1 - pnorm(80, 69, 3)))
```
...
c, AS seen in question 10, the normal approximation tends to overestimate the extreme values. It's possible that there are more seven footers than we predicted.
