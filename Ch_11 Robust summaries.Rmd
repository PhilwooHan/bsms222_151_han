...
8. How much can the average accidentally grow with mistakes like this? Write a function called error_avg that take a value k and returns the average of the vector x after the first entry changed to k. Show the results for k=10000 and k=-10000
```{r}
...
```{r}
reported_heights <- reported_heights %>% filter(!is.na(height))
```
...
max_height <- quantile(reported_heights$height, 0.75) + whisker
min_height <- quantile(reported_heights$height, 0.25) - whisker
reported_heights %>%
