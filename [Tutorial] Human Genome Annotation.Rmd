...
d1 = read_delim("gencode.v31.basic.annotation.gtf",
...
d1 = rtracklayer::import("gencode.v31.basic.annotation.gtf")
class(d1)
```
...
## 2.2. How mnay feature types in the GENCODE dataset?
...
```{r}
d1 = as.data.frame(d1)
d1 %>% filter(type == "transcript") %>% count(gene_id)  %>% pull(n) %>% mean()
```
```{r}
d1 %>% filter(type == "transcript") %>% count(gene_id)  %>% pull(n) %>% quantile( prob = c(0.25,0.5,0.75))
```
```{r}
d1 %>% filter(type == "transcript") %>% group_by(gene_id) %>% count(gene_id) %>% arrange(desc(n)) %>% .[1,]
```
...
```{r}
d1 %>% filter(type == "gene") %>% pull(width) %>% mean()
```
...
```{r}
d2 <- d1 %>% mutate(chrom_type = if_else(seqnames %in% c("chrX", "chrM", "chrY"), "sex", "autosomal"))

d2 %>% group_by(chrom_type) %>% filter(type == "gene") %>% summarise(mean = mean(width))
```
...
```{r}
d2 %>% group_by(gene_type) %>% summarize(distribution = quantile(width, probs = seq(0 ,1 , 0.25)))
```
...
```{r}
d2 %>% filter(type == "transcript" & transcript_support_level %in% c(1,2,3,4,5)) %>% group_by(gene_type) %>% count(transcript_support_level)
```

3. From the first question, please count the number of transcript for each TSL by source.

```{r}
d2  %>% filter(type == "transcript" &  transcript_support_level %in% c(1,2,3,4,5) ) %>% group_by(source) %>% count(transcript_support_level)
```
...
```{r}
d3 <- d2 %>% filter(type == "gene")
d3 <- d3 %>% mutate(isHgnc = ifelse(is.na(hgnc_id), 0, 1) , isCCDS = ifelse(is.na(ccdsid), 0, 1))
```

2. Please count the number of hgnc by gene biotypes.

```{r}
d3 %>% filter(isHgnc == "1") %>% count(gene_type)
```
...
```{r}
d3 %>% filter(isHgnc == 1) %>%count(level)
```
...
```{r}
d2 %>% group_by(gene_id) %>% filter(type == "transcript") %>% count() %>% ungroup() %>%
  top_n(1, n)
```
...
```{r}
d2 %>% filter(transcript_type %in% c("lncRNA", 'protein_coding')) %>% group_by(transcript_type) %>% summarize(quantile = quantile(width))
```

3. Please count the number of transcripts by chromosomes.

```{r}
d2 %>% group_by(seqnames) %>% filter(type == "transcript") %>% count()
```
...
```{r}
d3 %>% group_by(seqnames) %>% count()
```

2. Please compare the number of genes between autosomal and sex chromosome (Mean, Median).
```{r}
sex <- c("chrX", "chrY", "chrM")
d3 %>% group_by(chrom_type) %>% count(seqnames) %>% summarize(mean = mean(n), median = median(n))
```

3. Please divide the genes into groups 'protein coding' and 'long noncoding', and then compare the number of genes in each choromosomes within groups.

```{r}
d3 %>% filter(gene_type %in% c("lncRNA","protein_coding")) %>% group_by(gene_type) %>% count(seqnames)
```
