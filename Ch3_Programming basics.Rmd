...
if(murder_rate[ind] < 0.25){
  print(murders_state[ind])
} else{
  print("no state has murder rate that low")
}
```
A related function that is very useful is ifelse. This function takes three qrguments: a logical and two possible answers. If the logical is TRUE, the value in the second argument is returned and if FALSE, the value in the third argument is returned. Here is an example:
```{r}
a <- 0
ifelse(a > 0, 1/a, NA)
```
The functions is particularly useful because it works on vectors. It examines each entry of the logical vector and returns elements from the vector provided in the third argument, if the entry is FALSE.
```{r}
a <- c(0, 1, 2, -4, 5)
...
sum(is.na(no_nas))
```
Two other useful functions are any and all. The any function takes a vector of logicals and returns TRUE if any of the entries is TURE. The all function takes a vector of logicals and returns TRUE if all of the entries are TURE. Here is an example:
```{r}
z <- c(TRUE, TRUE, FALSE)
any(z)
all(z)
...
```
Notice that variables defined inside a function are not saved in the workspace. So while we use s and n when we call avg, the values are created and changed on ly during the call. Here is an illustrative example:
```{r}
s <- 3
avg(1:10)
s
...
```{r}
## my_functions <0 function(VARIABKE_NAME){
...
## intersect, sediff, setequal, union
```
...
These function live in different *namespaces* R will follow a certain order when searching for a function in these *namespaces*. You cna see the order by typing:
```{r}
search()
```
...
stats::filter
```
If we want to be absoulutely sure that we use the **dplyr** filter, we can use
```{r}
dplyr:: filter
```
...
Now we can create a plot to search for a patter:
```{r}
n <- 1:m
plot(n, s_n)
```
If you noticed that it appears to be a quadratic, you are right track because the formula is $n(n+1)/2$
...
```
Other functionals are apply, lapply, tapply, mappy, vapply, and replicate. We mostly use sapply, apply, and replicate in this book, but we recommend familiarizing yourselves with the others as they can be very useful.
...
```
4. Create a functions sum_n that for any given value, say $n$, computes the sume of the integers from 1 to n (inclusive). Use the function to determine the sume of integers from 1 to 5,000.
```{r}
sum_n <- function(n){
  sum(1:n)
}
...
x
```
7. Write a function compute_s_n that for any give $n$ computes the sum $S_n = 1^2 + 2^2 + 3^2 + \dots n^2$. Report the value of the sum whe n =10
```{r}
compute_s_n <- function(n){
  x <- 1:n
  sum(x^2)
