...
heights %>%
  ...
  geom_histogram(bindwidth = 1, col = "black") +
  facet_grid(sex~.)
```
...
ggplot(aes(life_expectancy_2010,life_expectancy_2015,label=country))+
geom_point()+
geom_text_repel() +
geom_abline(lty=2)
```
...
For these exercises, we will be using the baccines data in the *dslabs* package:
...
Which plot is easier to read if you are interested in determining whcih are the best and worst states in terms of rates, and why?
...
```{r}
dat$state
reorder(dat$state, dat$rate) %>% levels()
```
...
avg <- us_contagious_diseases %>%
  ...
  summarize(us_rate = sum(count, na.rm = T) / 
...
dat %>%
  ...
  geom_line(aes(year, rate, group = state), color = "grey50",
            show.legend = F, alpha = 0.2, size = 1) +
...
```{r}
dat <- us_contagious_diseases %>%
  filter(disease == "Smallpox" & weeks_reporting > 43) %>%
  mutate(rate = count / population * 10000 * 52 / weeks_reporting)

avg <- dat %>% group_by(year) %>%
  summarize(mean = sum(count, na.rm = T)/ sum(population, na.rm = T) * 10000)


dat %>% ggplot() +
  geom_line(aes(year, rate, group = state), color = "gray50",
            alpha = 0.4, size = 1) +
  geom_line(data = avg, mapping = aes(year, mean), size =1) +
  scale_y_continuous(trans = "sqrt", breaks = c(1,4, 9,16)) +
  ggtitle("Cases for 10,000 by state") +
  geom_text(data = data.frame(x = 1938, y = 1.5),
            mapping = aes(x, y, label = "US average"), color= "black") +
  xlab("") + ylab("")
```

3. For the state of California, make a time series plot showing rates for all diseases. Include only years with 10 or more weeks reporting. Us a different color for each disease.
...
cali %>% ggplot(aes(year, rate, col = disease)) +
...
us %>% ggplot(aes(year, rate, color = disease)) +
  ...
  theme(legend.position = "bottom") +
  xlab("") + ylab("") +
  ggtitle("US contagious diseases rates per year")
```
