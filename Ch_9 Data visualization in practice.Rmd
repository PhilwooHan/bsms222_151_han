---
title: "Chpater 9 Data visualization in practice"
Author: Philwoo, Han
output:
  html_document:
    df_print: paged
...
ut which line goes with which country? We can assign colors to make this distinction. A useful side-effect of using the color argument to assign different colors to the different countries is that the data is automatically grouped:
...
```{r}
gapminder <- gapminder %>% mutate(dollors_per_day = gdp/population/365)
```
...
gapminder %>%
  ...
  ggplot(aes(dollors_per_day)) +
  geom_histogram(binwidth = 1, col = "black")
```

We use the color = "black" argument to draw a boundary and clearly distinguish the bins.

In this plot, we see that for the majority of countries, averages are below 10 dollar a day. However, the majority of the x-axis is dedicated to the 35 countries with averages above 10 dollar. So the plot is not very informative about countries with values below $10 a day.
...
gapminder %>%
  filter(year == past_year & !is.na(gdp)) %>%
  ggplot(aes(log2(dollors_per_day))) +
...
### 9.5.3. Transfomr the values or the scale?
...
gapminder %>%
  filter(year == past_year & !is.na(gdp)) %>%
  mutate(region = reorder(region, dollors_per_day, FUN = median)) %>%
  ggplot(aes(dollors_per_day, region)) +
  geom_point() +
  scale_x_continuous(trans = "log2")
```
...
p <- gapminder %>%
  filter(year == past_year & !is.na(gdp)) %>%
  ggplot(aes(group, dollors_per_day)) +
  geom_boxplot() +
...
library(ggridges)
p <- gapminder %>%
  filter(year == past_year & !is.na(dollors_per_day)) %>%
  ggplot(aes(dollors_per_day, group)) +
  scale_x_continuous(trans = "log2")
...
country_list_2 <- gapminder %>%
  filter(year == present_year & !is.na(dollors_per_day)) %>%
  pull(country)
...
gapminder %>%
  ...
  ggplot(aes(group, dollors_per_day, fill = year)) +
  geom_boxplot() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  scale_y_continuous(trans = "log2") +
  xlab("")
...
gapminder %>%
  ...
  ggplot(aes(dollors_per_day, fill = group)) +
  scale_x_continuous(trans = "log2") +
...
p <- gapminder %>%
  filter(year %in% years & country %in% country_list) %>%
  mutate(group = ifelse(group == "West", "West", "Developing")) %>%
  ggplot(aes(dollors_per_day, y = ..count.., fill = group)) +
...
```{r}
gapminder %>%
  filter(year %in% years & !is.na(dollors_per_day)) %>%
  ggplot(aes(dollors_per_day, group)) +
  scale_x_continuous(trans = "log2") +
