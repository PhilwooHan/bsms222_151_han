...
(-b - sqrt(b^2 - 4*a*c)) / (2*a)
```
by creating and saving a script with the code above, we would no need to retype everything each time and instead, simply change the variable names. Try writing the script above into an editor and notice how easy it is to change the variables and receive an answer.


### 2.2.8 Commenting your code

If a line of R code starts with the symbol #, it is not evaluated. We can us this to write reminders of why we wrote particular code. For example, in the script above we could add:
```{r}
...
1. What is the sume of the first 100 positive integers? The formulat for the sum of integers $1$ through $n$ is $n(n + 1) / 2$ Define $n = 100$ and then use R to compute the sume of $1$ through $100$ using the formula. What is the sum?
```{r}
n <- 100
(n * (n + 1)) / 2
```
2. Now use the same formula to compute the sume of the integers from 1 through 1,000.
```{r}
n <- 1000
(n * (n+1)) / 2
```
...
4. In math and programming, we say that we evaluate a function when we replace the argument with a given number. So if we type sqrt(4), we evaluate the sqrt function. In R, you can evaluate a function inside another function. The evaluations happen from the inside out. Use one line of code to compute th log, in base 10, of the square root of 100.
```{r}
...
Variables in R can be different types. For example, we need to distinguish numbers from chracter strings and tables from simple lists of numbers. The function class helps us determine what type of object we have:
```{r}
a <- 2
class(a)
```
...
Up to now, the variables we have defined are just one number. This is not very useful for storing data. The most common way of storing a dataset in R is in a *data frame*. Conceptually, we can think of a data frame as a table with rows representing observations and the different variables reported for each observation defining the colums. Data frames are particularly useful for datasets becasue we can combine different data types into one object.
A large proportion of data analysis challenges start with data stored in a data frame. For example, we stored the data for our motivating example in a data frame. You can access this dataset by loading the **dslabs** libary and loading the murders dataset using the data function:
```{r}
library(dslabs)
data(murders)
```
...
Before we go any further in answering our orginal question about different states, lets learn more about the components of this object.

### 2.4.3 The accessor: $

For our analysis, we will need to access the different variables represented by colums included in this data frame. To do this, we use the accessor operator $ in the following way:
```{r}
murders$population
```
...
names(murders)
```
It is important to know that the order of the entries in murders$population preserves the order of the rows in our data table. This will later permit us to minipulate one variable based on the results of another. For example, we will be able to order the stated names by the number of murders.
**Tip**: R comes with a very nice auto-complete functionality that saves us the trouble of typing out all the names. Try typing murder$p the hitting the *tab* key on you keyboard. This functionality and many other useful auto-complete features are avilable when working in RStudio.
...
**Advanced**: Mathematically, the values in pop are integers and there is an integer class is R. However, by default, numbers are assigned class numeric even when they are round integers. For example, class(1) retruns numeric. You can turn them into class integer with the as.integer() function or by adding an L like this: 1L. Note the class by typing: class(1L)
...
```
It is a *factor*. Factors are useful for storing categorical data. We can see that tehre are only 4 regions by using the levels function.
```{r}
...
**Warning**: Factors can be a source of confusion since sometimes they behave like characters and sometimes they do not. As a result, confusion factors and characters are a common source of bugs.
...
You miight also encounter lists without variable names.
```{r}
record2 <- list("John Doe", 1234)
record2
```
If a list deos not have names, yhou cannot extract the elements with $, but you can still use the brackets method and instead of providing the variable name, you provide the list index, like this
```{r}
record2[[1]]
```
...
We can define a matrix using the matrix function. We need to specify the number of rows and colums.
```{r}
mat <- matrix(1:12, 4, 3)
mat
```
...
Similarly, if you want the entire third column, you leave the row spot emppty:
```{r}
mat[ , 3]
```
This is also a vector, not a matrix.
You can access more than one colun or more than one row if you like, This will give you a new matrix.
```{r}
mat[ , 2:3]
```
...
data(murders)
```
Use the function str to examine the structure of the murders object. Which of the following best describes the variables represented in the data frame?
```{r}
str(murders)
```
c. The state name, the abbreviation of the state name, the state's region, and the states's population and total number of murders for 2010.
...
class(a)
```
4. Now use the square brackets to extract the state abbreviations and and assign them to the object b. Use the identical function to determine if a and b are the same.
```{r}
b <- murders[["abb"]]
a == b
identical(a, b)
...
class(murders$region)
```
With on line of code, use the function elvels and length to determine the number of regions defined by the dataset.
```{r}
length(levels(murders$region))
```
6.The functions table takes a vector and returns the frequency of each element. You can quickly see how many states are in each region by applying this function. Use this function in one line of code to create a table of states per region.
```{r}
...
In R, the most basic objects available to store data are *vectors*. As we have seen, complex datasets cna usually be broken down into components that are vectors. For example, in a data frame, each column is a vector. Here we learn more about this important class.
...
##country <- c(italy, canada, egypt)
```
you receive an error because the variables italy, cnada, and egypt are not defined. If we do not use the quotes, R looks for variables with those names and returns an error.
...
names(codes)
```
If we use of strings without quotes looks confusing, know that you can use the quotes as well:
```{r}
codes <- c("italy" = 380, "canada" = 124, "egypt" = 818)
codes
```
...
seq(1, 10)
```
The frist argument defines the start, and the second defines the end which is included. The default is to go up in increments of 1, but a third argument lets us tell it how much to jump by:
```{r}
seq(1, 10, 2)
```
...
1:10
```
When we us these functions, R produces integers, not numerics, because they are typically used to index something:
```{r}
class(1:11)
```
...
We use square brackets to accss specific elements of a vector. For the vector codes, we defined above, we can access the second element using
```{r}
codes[2]
```
...
In general, *coercion* is an attempt by R to be Flexible with data types. When an entry does not match the expected, some of the prebuilt R functions try to guess what was meant before throwing an error. This can also lead to confusion. Failing to understand *coercion* can drvie programmers crazy when attempting to code in R since it behaves quite differently from most other languages in this regard. Let's learn about it with some examples.
...
When a functions tries to coerce one type to another and encounters an impossible case, it usually gives us a warning and turns the entry into a special value called an NA for "not available.: For example:
```{r}
x <- c("1", "b", "3")
as.numeric(x)
```
R does not have any guesses for what number you want when you type b, so it does not try.
As a data scientist you will encounter the NA s often as they are generally used for missing data, a common porblem in real-world datasets.
