...
colnames(d_luad)
```
Do wee see the same columns for the LUSc dataset?
...
```
Q1: It complete the data.frame by type. Make another row even n = 0.

Q2" ncol determine how many columns for each row. Scales: Should scales be fixed ("fixed", the default), free ("free"), or free in one dimension. - each column get free scale size.
...
```{r}
d_luad %>% ggplot(aes(age_at_diagnosis)) +
...
```{r}
d_luad %>%
  ggplot(aes(age_at_diagnosis/365)) +
  geom_histogram(bins = 100)
```
...
```{r}
d_luad %>%
  ggplot(aes(age_at_diagnosis/365, fill = gender)) + 
  geom_histogram(bins = 100)
```
...
Can you tell the difference between boxplot?
- From boxplot it is hard to find wether it is bimoidality.
...
In violin plot we can find the existence of bimodality

```{r}
d_luad %>%
  ggplot(aes(gender, age_at_diagnosis/365)) +
...
d_luad %>%
  ...
  labs(x = "Age at Diagnosis(years)", y = 'Number of LUAD patients',
       tilte = 'TCGA: Lung cancer adenocarcinoma') +
  geom_histogram(bins = 100)
```

```{r}
 d_luad %>% 
  mutate(smoke_status = ifelse(is.na(years_smoked), "non-smoker", "smoker")) %>%
  ggplot(aes(smoke_status, intermediate_dimension, fill = smoke_status)) +
  geom_violin() +
  geom_boxplot(fill = "white", width = .25) +
  labs(x = "", title = "smoker vs non-smoker in lung cancer dimension")

```

```{r}
d_luad %>% mutate(smoke_status = ifelse(is.na(years_smoked), "non-smoker", "smoker")) %>%
  ggplot(aes(intermediate_dimension, fill = smoke_status)) +
...
bind_rows(d_luad %>%
          ...
          d_lusc %>%
            ...
            mutate(type = "lusc") %>%
            select(type, tumor_stage)) %>% 
  count(type, tumor_stage) %>%  mutate(tumor_stage = factor(tumor_stage, levels=rev(unique(tumor_stage)))) %>%  ggplot(aes( tumor_stage, n, fill=type)) +   labs(title = 'Cancer type by tumor stages',       x = '', y='Number of samples for TCGA RNA-seq') +   theme_minimal() + geom_bar(stat="identity") +   facet_wrap(~type) + coord_flip() 
```
...
bind_rows(d_luad %>%
          ...
          d_lusc %>%
            ...
            select(type, gender, age_at_diagnosis, tissue_or_organ_of_origin)) %>% 
  ggplot(aes(gender, age_at_diagnosis/365, fill = gender)) +
  geom_boxplot() + facet_wrap(~tissue_or_organ_of_origin) + labs(y="year at diagnosis")
```
