...
The summarize function in *dplyr* provides a way to compute summary statistics with intuitive and readable code. We start with a simple example based on heights. The hiegts dataset includes heights and sex reported by students in an in-class survey.
...
```
This is because in the computation above the small states are give the same weight as the large ones. The US murder rate is the total number of murders in the US divided by the total US population. So the correct computation is:
```{r}
...
But we can obtain these three values with just one line using the quantile funtion: quantile(x, c(0.5, 0, 1)) returns the median (50th percentile), the min (0th percentile), and max (100th percentile) of the vector x. We can us it with summarize like this:
```{r}
heights %>%
  filter(sex =="Female") %>%
...
```
The results does not look very different from heights, except we see Groups: sex[2] when we print the object. Although not immediately obvious from its appearance, this is now a special data frame called a **grouped data frame**, and *dplyr* functions, in particular summarize, will behave differently when acting on this object. Conceptually, you can think of this tables, with the same columns but not necessarily the same number of rows, stacked together in one object. When we summarize the data after grouping, this is what happens:
```{r}
heights %>%
  group_by(sex) %>%
  summarize(average = mean(height), standard_deviation = sd(height))
...
For these exercises, we will be using the data from the survey collected by the United States National Center for Health Statistics(NCHS). This center has conducted a series of health and nutrition surveys since the 1970's. Starting in 1999, about 5,000 individuals of all ages have been interviewed every year and they complete the health examination component of the survey. Part of the ata is made available via the *NHANES* packages. Once you install the *NHANES* package, you can load the data like this:
```{r}
library(NHANES)
data(NHANES)
```
...
```
Let's now explre the NHANES data

1. We will provide some basic facts about blood pressure. First let's select a group to set the standard. We will use 20-to29-year-old femanles. AgeDecade is an categorical variable with these ages. Note that the category is coded like " 20-29", with a space in front! What is the average and standard deviation of systolic blood pressure as saved in the BPSysAve variable? Save it to a variable called ref.
...
ref <- NHANES %>% 
  ...
  summarize(mean = mean(BPSysAve, na.rm = T), standard_deviation = sd(BPSysAve, na.rm = T))
```

2. Using a pipe, assign the average to a numeric variable ref_avg. Hint: Use the code similar to above and then pull.

```{r}
ref_avg <- ref %>% pull(mean, standard_deviation)
```
...
NHANES %>%
  filter(AgeDecade == " 20-29") %>%
  summarize(min= min(BPSysAve, na.rm = T), max = max(BPSysAve, na.rm= T))
```

4. Compute the average and standard deviation for females, but for each age group separately rather than a selected decade as in question 1. Note that the age groups are defined by AgeDecade. Hint: rather than filtering by age and gender, filter by Gender and the use group_by.
```{r}
NHANES %>% 
...
7. For males between the ages of 40-49, compare systolic blood pressure across race as reproted in the Race1 variable. Order the resulting table from lowest to highest average systolic blood pressure.
```{r}
NHANES %>% 
  filter(Gender == "male") %>%
  filter(AgeDecade == " 40-49") %>%
  group_by(Race1) %>%
  summarize(mean = mean(BPSysAve, na.rm = T), sd = sd(BPSysAve, na.rm= T)) %>%
  arrange(mean)
```
...
```
The tbl, pronounced tibble, is a special kind of data frame. The functions group_by and summarize always return this type of data frame. The group_by function returns a special kind of tbl, the grouped_df. We will say more about these later. For consistency, the *dplyr* manipulation verbs( select, filter, mutate, and arrange) preserve the class of the input: If they recieve a regular data frame they return a regular data frame, while if they receive a tibble they return a tibble. But tibbles are the preferred format in the tidyverse and as a result tidyverse functions that produce a data frame from scratch return a tibble. For example, in Chapter 5 we will see that tidyverse functions used to import data create tibbles.
Tibbles are very similar to data frames. In fact, you can think of them as a modern version of data frames. nonetheless there are three important differences which we describe nest.

### 4.11.1 Tiibbles display better
...
If you subse the columns of a data frame, you may get back an object that is not a data frame, such as a vector or scalar. For example:
```{r}
class(murders[,4])
```
...
murders$Population
```
retruns a NULL with no warning, which can make it harder to debug. In contrast, if we try this with a tibble we get an informative warning:
```{r}
...
While data frame columns need to be vetors of numbers, strings, or logical values, tibbles can have more complex objects, such as lists or functions. Also, we can create tibbles with functions:
```{r}
...
### 4.11.5 Creat a tibble using tibble instead of data.frame
...
class(s_n)
```
If we want a numeric vetor, we can instead use map_dbl which always returns a vector of numeric values.
```{r}
s_n <- map_dbl(n, compute_s_n)
class(s_n)
```
This produces the same results as the sapply call shown above.
A particularly useful *purrrr* function for interacting with the rest of the tidyverse is map_df, which always returns a tibble data frame. However, the function being called needs to return a vector or a list with names. For this reason, the following code would result in a Argument 1 must have names error:
```{r}
...
s_n <- map_df(n, compute_s_n)
```
The *purrr* package provides much more functionality not covered here. For more details you can consulte this online resource.

## 4.14 Tidyverse conditionals

A typical data analysis will often involve one or more conditional operations. In section 3.1 we described the ifelse function, which we will use extensively in this book. In this section we present tow *dplyr* functions that provide further functionality for performing conditional operations.
...
case_when(x <0 ~ "Negative",
          ...
          TRUE ~ "Zero")
```
A common use for this functions is to define categorical variables based on existing variables. For example, suppose we want to compare the murder rates in four groups of states: **New England, West Coast, South**, and **other**. For each state, we nned to ask if it is in New England, if it is not we ask if it is in the West Coast, if not we ask if it is in the South, and if not we assign **other**. Here is how we use case_when to do this:
```{r}
murders %>%
...
A common operation in data analysis is to determine if a value falls inside an interval. We can check this using conditionals. For example, to check if the elements of a vector x are between a and b we can tybe
```{r}
##x >= a & x <= b
```
However, this can becom cumbersome, especially within the tidyverse approach. The between function performs the same operation
```{r}
## between(x, a, b)
```
...
1. Load the murders dataset. Whcih of the following is true?
```{r}
class(murders)
```
...
```{r}
n <- 1:100
compute_n <- function(n){
  tibble(n = n, s_n = sum(1:n), s_n_2 = sum(1:n))
}
map_df(n, compute_n)
```
