...
Many other approaches are available for creating plots in R. In fact, the plotting capabilities that come with a basic installation of R are already quite powerful. There are also other packages for creating graphics such as *grid* and *lattice*. We chose to use *ggplot2* in this book because it breaks plots into components in a way that permits beginners to create relatively complex and aesthetically pleasing plots using syntax that is intuitive and comparatively easy to remeber.

One reason *ggplot2* is generallly more intuitive for beginners is that it uses a grammer of graphics, the **gg** in *ggplot2*. This is analogous to the way learning grammar can help a beginner construct hundreds of different sentences by learning just a handful of verbs, nouns and adjectvies without having to memorize each specific sentence. Similarly, by learning a handful of *ggplot2* building blocks and its grammar, you will be able to create hundreds of different plots.
...
We will construct a graph that summarizes the US murders datset that looks like this:

We can clearly see how much states vary across population size and the total number of murders. Not suprisingly, we also see a clear relationship between murder totals and population size. A state falling on the dashed grey line has the same murder rate as the US average. The four geographic regions are denoted with color, which depicts how most southern states have murder rates above the average.
...
The first step in learning *ggplot2* is to be albe to break a graph apart into components. Let's break down the plot above and introduce some of the *ggplot2* terminology. The main three components to note are:
...
- There are labels, a title, a legend, and we use the style of The Economist megazine.
...
We can also pie the data in as the first argument. So this line of code is equivalent to the one above:

```{r}
murders %>% ggplot()
```

It renders a plot, in this case a black slate since no geometry has been defined. The only style choice we see is a grey background.
...
class(p)
```
To render the plot associated with this object, we simply print the object p. The following two lines fo code each produce the same plot we see above
```{r}
print(p)
p
```
...
Taking a quick look ate the cheat sheet, we see that the function used to create plots wit this geometry is geom_point.

(Image courtesy of RStudio. CC-BY-4/0 license.)

Geometry function names follow the pattern: geom_X where X is the name of the geometry. Some examples includeinclude geom_point, geom_bar, and geom_histogram.
...
#geom_point understands the following aesthetics (regquired aesthetics are in bold):
...
*Aesthetic mappings* describe how properties of the data connect with feactures of the graph, such as distance along an axis, size, or color. The aes function connects data with what we see on the graph by defining aesthetic mappings and will be one of the functions you use most often when plotting. The outcome of the aes function is often used as the argument of a geometry functin. This example produces a scatterplot of total murders verssus population in millions:
```{r}
murders %>% ggplot() +
...
Because each point(each state in this case) has a label, we need an aesthetic ampping to make the connection between points and labes. By reading the help file, we learn that we supply the mapping between point and label through the label argument of aes. So the code looks like this:
...
First, our desired scales are in log-scale. This is not the default, so this change needs to be added through a **scales** layer. A quick look at the cheat sheet reveals the scale_x_continous function lets us control the behavior of scales. We use them like this:
```{r}
p + geom_point(size = 3) +
...
```
We are alomost there! All we have left to do is add color, a legend, and optional changes to the style.
...
p <- murders %>% ggplot(aes(population/10^6, total, label = abb)) +
  ...
  ylab("Total number of murdrers (log scale)") +
  ggtitle("US Gun Murders in 2010")
```
...
Since the choice of color is determined by a feature of each obervation, this is an aesthetic mapping. To map each point to a color, we need to use aes. We us the following code:

```{r}
p + geom_point(aes(col = region), size = 3)
```

The x and y mappings are inherited from those already defined in p, so we do not redefine them. We also move aes to the fist argument since that is where mappings are expected in this function call.
...
For example, we can make changes to the legend via the sacle_color_discrete function. In our plot the word **region** is capitalized and we can change it like this:
...
The style of a *ggplot2* graph can be changed using the theme functions.Several themes are included as part of the *ggplot2* package. In fact, for most of the plots in this book, we use a function in the *dslabs* packages that automatically sets a default theme:
```{r}
ds_theme_set()
```

Many other themes are added by the packages *ggtehmes*. Among those are the theme_economist theme that we used. After installing the package, you can change the style by adding a layer like this:
...
You can see how some of the ohter themes look by simply changing the function. For instance, you might try the theme_fivethirtyeight() theme instead.

The final difference has to do with the postion of t he labels. In our plot, some of the labels fall on top of each other. The add-on packages *ggrapel* includes a geometry that adds labels while ensuring that they don't fall on top of each other. We simply change geom_text with geom_text_repel.
...
murders %>% ggplot(aes(population/10^6, total, label = abb)) +
  ...
  scale_y_log10() +
  xlab("Populations in millions (log scale)") +
  ylab("Total number of mureders (log scale)") +
  ggtitle("US Gun murders in 2010") +
...
data.frame(x = x, y = y) %>%
  ...
  geom_point()
```
This seems like too much code for such a simple plot. The qplot function scacrifices the felxibility porvided by the ggplot approach, but allows us to generate a plot quickly.
```{r}
qplot(x, y)
```
...
There are often reasong to graph plots next to each other. The *grideExtra* package permits us to do that:
...
1. With *ggplot2* plots can be saved as objects. For example we can associate a datset with a plot object like this
```{r}
p <- ggplot(data = murders)
```
...
3. Using the pipe %>%, create an object p but this time associated with the heights datset instead of the murders dataset.
```{r}
p <- heights %>% ggplot()
```
...
c. totla and population

6. To create the scatterplot we add a layer with geom_point. The aesthetic mappings require us to define the x-axis and y-axis variables, repectively. So the code looks like this:
```{r}
...
8. IF instead of points we want tpo add text, we can use the geom_text() or geom_label() geometries. The following code
```{r}
...
14. Now we are going to change the x-axis to a log scale to account for the fact the distribution of population is skewed. Let's tart by defining an object p holding the plot we have made up to now.
```{r}
p <- murders %>%
